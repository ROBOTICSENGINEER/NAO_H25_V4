...
{
    int i,j,k;
    float xt=0.0000,yt=2.0000,phi0=0.0000;
    ...
    //ALMotionProxy::setSmartStiffnessEnabled(const bool& enable);
    ...
    while(E2>0.04)
    {
        ...
        if(see_target==2)
        {
            ...
            for(j=0;j<5;j++)
            {
                for(i=0;i<5;i++)
                {
                    R[i+1][j+1]=5*pb[i][j]-5*pr[i][j]-0.1*pg[i][j];
                }
            }
